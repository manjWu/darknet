...
{
	//printf("start to forward convolutional layer\n");
...
#if 1
	//It's same with weights value, change all "delta" to "weights"
	char filename[30];
	sprintf(filename, "delta_%d.txt", l.index);             
...
{
    //printf("start to backward convolutional layer\n");
    if(l.smooth){
